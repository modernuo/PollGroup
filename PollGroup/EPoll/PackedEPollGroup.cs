...
{
    internal sealed class PackedEPollGroup<T> : IPollGroup where T : IPackedArch
    {
        private readonly int _epHndle;
        private epoll_event_packed[] _events;

        public PackedEPollGroup()
        {
            _events = new epoll_event_packed[2048];
        ...
        public int Poll(int maxEvents)
        {
            if (maxEvents > _events.Length)
            {
                ...
                _events = new epoll_event_packed[newLength];
            }
